...
| Codex | **`$STAGING/codex-home/config.toml`** | Legacy **`commit_attribution`** stripped (removed in modern Codex versions; avoids unrecognized setting errors); **`daemon_auto_start = false`** under **`[features]`** prevents daemon socket failures. |
