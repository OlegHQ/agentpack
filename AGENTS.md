...
| Codex | **`$STAGING/codex-home/config.toml`** | Legacy **`commit_attribution`** stripped (removed in modern Codex versions; avoids unrecognized setting errors); daemon socket path length is safely constrained within `SUN_LEN` limits. |
