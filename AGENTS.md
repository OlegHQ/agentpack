...
10. **Launchers**
   ...
   - **`agentpack grok`** runs **`grok`** with **`GROK_HOME=$STAGING/grok-home`**. It does **not** inject **`--cwd`** (the child inherits the shell's cwd, like `agentpack claude` / `opencode`) and does **not** pass **`--plugin-dir`** or **`--settings`** for `grok 0.1.219`.
   - **`agentpack agent`** runs Cursor Agent with **`HOME=$STAGING/cursor-home`**. The default binary name is **`cursor-agent`** (Cursor's official install name) so it does not collide with the unrelated grok-shipped **`agent`** binary on **`PATH`**; override with **`CURSOR_AGENT_PATH`** if needed. **`--workspace`** is **not** auto-injected — the child inherits the shell's cwd, like `agentpack claude` / `opencode`. **`CURSOR_CONFIG_DIR`** is **`$HOME/.cursor`** under the fake home. **Workspace trust** uses **`$CURSOR_DATA_DIR/projects/<slug>/.workspace-trusted`**; **`agentpack`** sets **`CURSOR_DATA_DIR`** to **real `~/.cursor`** when unset so trust state is not lost when **`$STAGING`** is recreated. It also preserves **`CARGO_HOME`**, **`RUSTUP_HOME`**, and **`DOCKER_CONFIG`** from the real home unless those env vars are already set. For a **stable** staging path when your OS rotates temp dirs, set **`AGENTPACK_STAGING_ROOT`**. Cursor’s **`cursor-agent`** only accepts **`--trust`** with **`--print`** / headless; **`agentpack`** prepends **`--trust`** automatically in that case.
   - **`agentpack agy`** runs **`agy`** without injecting **`--add-dir`** (the child inherits the shell's cwd) and leaves **`HOME`** / **`~/.gemini`** untouched.
