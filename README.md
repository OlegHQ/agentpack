...
# Add a plugin with a version pin
agentpack add anthropics/claude-plugins-official/plugins/hookify --version "^1.0.0"

# Launch Claude Code with everything bundled
agentpack claude

# Or launch other agents
...
4. **Launchers** — `agentpack claude`, `agent`, `opencode`, `codex` each start the target agent with the staged bundle injected via the agent native extension mechanism.

### Cross-Harness Artifact Conversion

agentpack does not just copy files — it **converts** between formats automatically:
