...
//!
//! Pointing `CLAUDE_CONFIG_DIR` at per-project staging would make the keychain service name (and
//! file-fallback path) change on every project/mode switch and macOS reboot, so Claude forgets the
//! login each time. Instead we leave `CLAUDE_CONFIG_DIR` unset and pass `claude --settings <path>`
//! with our attribution-off overlay; Claude loads that as `flagSettings` (precedence above
//! user/project/local) and credentials stay at the user-global keychain entry.
//!
//! The overlay lives at `$AGENTPACK_HOME/claude-settings.json` so all projects share it. It also
//! pre-approves MCP servers staged into the bundle's `.mcp.json` via `enabledMcpjsonServers`,
//! otherwise Claude drops them as untrusted project-scope MCPs.
...
/// Set `enabledMcpjsonServers` to exactly `names` in the agentpack `--settings` overlay. This
/// pre-approves only the staged MCP names; other `.mcp.json` servers still require interactive
/// approval.
